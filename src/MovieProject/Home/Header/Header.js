      ...
      <div className={"HeaderText"}>
        <div className={"HeaderTextLeft"}>
          <h1 className={"headerTitle"}>
            {movies.title?.length > 30
              ? movies.title.substr(0, 30) + "..."
              : movies.title}
          </h1>
          <p className={"headerParagraph"}>
            {movies.overview?.length > 170
              ? movies.overview.substr(0, 170) + "..."
              : movies.overview}
          </p>
        </div>
        <div className={"HeaderTextRight"}>
          <h4 className={"headerLanguage"}>
            {movies.original_language === "en" ? "English" : "Russian"}
          </h4>
